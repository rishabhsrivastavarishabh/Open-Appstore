...
export interface HomeData {
  ...
  stats: { total_apps: number; total_downloads: number; avg_rating: number; developers: number }
}

/** ===================== HOME (server-rendered HTML first page) ===================== */
...
<section class="hero" id="hero-section">
  ...
  <div class="hero-content">
    <p class="hero-eyebrow"><i class="fa-solid fa-sparkles"></i> ${d.stats.total_apps} apps · ${formatCount(d.stats.total_downloads)} downloads · ${d.stats.developers} developers</p>
    ...
    <div class="hero-quick">
      ...
      ${activeCats
        .slice(0, 5)
        .map((c) => `<a href="/apps?category=${encodeURIComponent(c.name)}" class="quick-chip">${esc(c.name)}</a>`)
        .join('')}
    </div>
    <div class="hero-ctas">
  ...
  ${
    spotlight
      ? `<aside class="hero-spotlight" aria-label="Featured app">
...
/** ===================== BROWSE / SEARCH ===================== */
export function browsePage(opts: {
  apps: AppView[]
  categories: { name: string; count: number }[]
  search: string
  category: string
...
<section class="browse-layout">
      ...
      <div class="filter-block">
        ...
        <div class="filter-chips">
          ...
          ${activeCats
            .map(
              (c) =>
                `<label class="chip-radio"><input type="radio" name="category" value="${esc(c.name)}" ${category === c.name ? 'checked' : ''} /><span>${esc(c.name)} <b>${c.count}</b></span></label>`
            )
            .join('')}
        </div>
      </div>
  ...
  <div class="browse-results">
    ...
    ${
      apps.length
        ? ''
        : emptyState('fa-magnifying-glass', 'No apps match those filters', 'Try a broader search or clear your filters.', {
            href: '/apps',
            label: 'Clear filters',
          })
    }
...
/** ===================== CATEGORIES ===================== */
export function categoriesPage(categories: { name: string; count: number }[]) {
...
<section class="page-head">
  <div class="page-head-inner">
    ...
    <p>${active.length} categories with published apps · ${categories.length} total</p>
  </div>
</section>
...
/** ===================== TOP CHARTS ===================== */
export function chartsPage(d: { popular: AppView[]; topRated: AppView[]; newest: AppView[]; free: AppView[] }) {
  const tab = (id: string, label: string, icon: string, apps: AppView[], ranked = true) => `
  <div class="chart-panel" id="chart-${id}" role="tabpanel" ${id === 'popular' ? '' : 'hidden'}>
    <div class="app-list app-list-bordered">
      ${apps.length ? apps.map((a, i) => appRow(a, ranked ? i + 1 : undefined)).join('') : emptyState('fa-chart-simple', 'No data yet', 'Charts populate as apps gain downloads and ratings.')}
    </div>
  </div>`
...
/** ===================== APP DETAIL ===================== */
export function appDetailPage(d: {
  app: AppView
  developer: any | null
  more: AppView[]
  similar: AppView[]
  ...
  const updated = app.updated_at ? new Date(app.updated_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : '—'
    ...
    <div class="app-hero-main">
      ...
      <div class="app-hero-info">
        ...
        <div class="app-hero-actions">
          <button class="btn btn-primary btn-lg js-get" data-app-id="${esc(app.id)}" data-app-name="${esc(app.name)}" data-url="${esc(app.download_url || app.website || '')}" data-slug="${esc(app.slug)}">
            <i class="fa-solid fa-download"></i> ${app.is_free ? 'Get it now' : `Buy $${app.price.toFixed(2)}`}
          </button>
          ${app.website ? `<a class="btn btn-outline-light btn-lg" href="${esc(app.website)}" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-globe"></i> Website</a>` : ''}
  ...
  <aside class="app-detail-side">
    <div class="card">
      ...
      <dl class="info-list">
        ...
        ${app.website ? `<dt>Website</dt><dd><a href="${esc(app.website)}" target="_blank" rel="noopener noreferrer">Visit site</a></dd>` : ''}
      </dl>
    </div>
...
/** Very small markdown subset renderer (safe: escapes first) */
...
export function developersPage(devs: any[]) {
  return raw(`
<section class="page-head">
  <div class="page-head-inner">
...
<section class="section">
  ${
    ...
    <div class="dev-tile-stats"><span><i class="fa-solid fa-cubes"></i> ${d.apps_count} apps</span><span><i class="fa-solid fa-download"></i> ${formatCount(d.total_downloads)}</span></div>
          ...
          .join('')}</div>`
      : emptyState('fa-users', 'No developers yet', 'Register a developer profile to be listed here.', { href: '/developer', label: 'Become a developer' })
  }
</section>
`)
}

export function developerProfilePage(d: { developer: any; apps: AppView[] }) {
...
/** ===================== LEGAL / STATIC ===================== */
export function legalPage(title: string, sections: { h: string; p: string[] }[]) {
  return raw(`
<section class="page-head">
  <div class="page-head-inner">
