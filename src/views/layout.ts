...
export const SITE_NAME = 'Open App Store'

export interface LayoutOptions {
  title: string
  description?: string
  mode?: 'store' | 'developer'
  active?: string
    ...
    <div class="primary-nav-inner">
      ${raw(
        nav
          .map(
            (n) =>
              `<a href="${n.href}" class="nav-link${o.active === n.key ? ' is-active' : ''}"><i class="fa-solid ${n.icon}" aria-hidden="true"></i><span>${n.label}</span></a>`
          )
          .join('')
      )}
    </div>
...
<footer class="site-footer">
  ...
  <div class="footer-bottom">
    <p>&copy; ${new Date().getFullYear()} Open App Store. Powered by Hono on Cloudflare + Supabase.</p>
...
<nav class="mobile-tabbar" aria-label="Mobile navigation">
  ${raw(
    (mode === 'developer'
      ...
      .map(
        (t) =>
          `<a href="${t.href}" class="tabbar-link${o.active === t.key ? ' is-active' : ''}"><i class="fa-solid ${t.icon}"></i><span>${t.label}</span></a>`
      )
      .join('')
  )}
</nav>
